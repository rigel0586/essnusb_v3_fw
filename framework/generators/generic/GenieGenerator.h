#ifndef ESBROOT_GENERATOR_GENERIC_GENIEGENERATOR_H
#define ESBROOT_GENERATOR_GENERIC_GENIEGENERATOR_H 1
...
//! configure Genie global state and to configure Genie generator backend using
//! user provided Genie flux and geometry interfaces.
//!
...
public:
  static struct GlobalState_t {
    ...
    std::string fXsecSplineFileName =
        ""; //!< Path to XML file with GENIE cross sections
    std::string fOutputFileName = ""; //!< Path to file used by genie generators
                                      //!< if they want to write data output
  ...
  void SetFluxI(std::shared_ptr<genie::GFluxI> FluxI) {
    assert(IsConfigured() == false);
    fFluxI = FluxI;
  };
  //! Set Genie geometry driver. Can't set if object already configured.
  void SetGeomI(std::shared_ptr<genie::GeomAnalyzerI> GeomI) {
    assert(IsConfigured() == false);
    fGeomI = GeomI;
  };
  ...
  std::shared_ptr<genie::GeomAnalyzerI> const &GetGeomI() const {
    return (fGeomI);
  };
...
//   virtual Bool_t ReadEvent(FairPrimaryGenerator *primGen);
  ...
  void AddPdgCode(std::vector<int> pdgCodes) {
    fpdgCodesAllowed.insert(fpdgCodesAllowed.end(), pdgCodes.begin(),
                            pdgCodes.end());
  }
...
protected:
  ...
  //! Check if the event particle is allowed by the chosen criteria fo the
  //! generator NOTE: if exclude = false , only pdg particles added in the
  //! fpdgCodesAllowed are valid 		if exclude = true, only particles NOT
  //! in fpdgCodesAllowed are valid
  ...
  virtual Bool_t
  KeepThrowing(std::vector<genie::GHepParticle *> &eventParticles);

  //! Write data from the event for further analysis if required
  //!  *@param event    generated event
  //!  *@param flaGkeepThrowing    flag indicating if this event is rejected and
