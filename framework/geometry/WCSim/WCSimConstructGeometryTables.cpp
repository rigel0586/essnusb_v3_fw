...
void WCSimGeometry::PrintGeometryTree
...
void WCSimGeometry::DescribeAndRegisterPMT(G4VPhysicalVolume* aPV ,int aDepth, int replicaNo,
                                                       const G4Transform3D& aTransform) 
{
...
void WCSimGeometry::DumpGeometryTableToFile()
{
  // Open a file
...
void WCSimGeometry::TraverseReplicas
...
void WCSimGeometry::DescribeAndDescendGeometry
(G4VPhysicalVolume* aPV ,int aDepth, int replicaNo, 
...
G4double WCSimGeometry::GetGeo_Dm(G4int i){
  if (i>=0&&i<=2){
    return WCCylInfo[i];
  }else if(i==3){
    return innerradius;
