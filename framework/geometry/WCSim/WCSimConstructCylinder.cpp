...
#include "WCSimTuningParameters.hpp" //jl145
...
G4LogicalVolume* WCSimGeometry::ConstructCylinder()
{
  ...
  G4LogicalVolume* logicWC = 
   ...
   logicWC->SetVisAttributes(G4VisAttributes(false)); //amb79
  ...
  if(isODConstructed) {
    ...
    //logicCaveTyvek->SetVisAttributes(G4VisAttributes(false)); //amb79
    ...
    //logicCaveCapsTyvek->SetVisAttributes(G4VisAttributes(false)); //amb79
...
// This volume needs to made invisible to view the blacksheet and PMTs with RayTracer
  if (Vis_Choice == "RayTracer")
   {logicWCBarrel->SetVisAttributes(G4VisAttributes(false));} 

  else
   {//{if(!debugMode)
		 G4VisAttributes* tmpVisAtt = new G4VisAttributes(G4VisAttributes(false));
  ...
  G4VPhysicalVolume* physiWCBarrelAnnulus = 
    new G4PVPlacement(0,
		      ...
		      0,true);
if(!debugMode)
   logicWCBarrelAnnulus->SetVisAttributes(G4VisAttributes(false)); //amb79
  //-----------------------------------------------------
...
if(!debugMode)
  ...
  logicWCBarrelRing->SetVisAttributes(G4VisAttributes(false));
  }
else {
        G4VisAttributes* tmpVisAtt = new G4VisAttributes(G4Colour(0,0.5,1.));
  ...
  if(!debugMode)
	...
	//If you want the columns on the Annulus to show in OGLSX, then comment out the line below.
  	logicWCBarrelCell->SetVisAttributes(G4VisAttributes(false));
	}
  else {
  	G4VisAttributes* tmpVisAtt = new G4VisAttributes(G4Colour(1.,0.5,0.5));
  	tmpVisAtt->SetForceWireframe(true);
...
if (Vis_Choice == "RayTracer"){
   ...
   G4VisAttributes* WCBarrelBlackSheetCellVisAtt 
	 ...
	 WCBarrelBlackSheetCellVisAtt->SetForceAuxEdgeVisible(true); // force auxiliary edges to be shown
      if(!debugMode)
		...
		logicWCBarrelCellBlackSheet->SetVisAttributes(G4VisAttributes(false));}
 ...
 // ---------------------------------------------------------
  ...
  if(!(WCBarrelRingNPhi*WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)){
    ...
    logicWCExtraTower->SetVisAttributes(G4VisAttributes(false));
  //-------------------------------------------
  ...
  //------------------------------------------
    ...
    G4VPhysicalVolume* physiWCTowerCell = 
      new G4PVReplica("extraTowerCell",
		      ...
		      (G4int)WCBarrelNRings-2,
		      barrelCellHeight);
    logicWCExtraTowerCell->SetVisAttributes(G4VisAttributes(false));
...
// These lines add color to the blacksheet in the extratower. If using RayTracer, comment the first chunk and use the second. The Blacksheet should be green.
  ...
  if (Vis_Choice == "OGLSX"){
   ...
   G4VisAttributes* WCBarrelBlackSheetCellVisAtt 
	...
	if(!debugMode)
	  {logicWCTowerBlackSheet->SetVisAttributes(G4VisAttributes(false));}
	else
...
If used here, uncomment the SetVisAttributes(WClogic) line, and comment out the SetVisAttributes(G4VisAttributes(false)) line.*/
  ...
  if(!(WCBarrelRingNPhi*WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)){
	...
	if(isODConstructed){
		...
		logicWCODCapTyvek->SetVisAttributes(G4VisAttributes(false));
		...
		logicWCBarrelCellODTyvek->SetVisAttributes(G4VisAttributes(false));
		//// Uncomment following for TYVEK visualization
    ...
    if(WCODPMTShift > barrelODCellWidth/2. - WCPMTODRadius) WCODPMTShift == 0.*cm;
		...
		if(!(WCBarrelRingNPhi*WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)){
			...
			logicWCTowerODTyvek->SetVisAttributes(G4VisAttributes(false));
			//// Uncomment following for TYVEK visualization
 ...
 // RayTracer
  if (Vis_Choice == "RayTracer"){
	logicBottomCapAssembly->SetVisAttributes(G4VisAttributes(false));
	logicTopCapAssembly->SetVisAttributes(G4VisAttributes(false));}
...
G4LogicalVolume* WCSimGeometry::ConstructCaps(G4int zflip)
{
  ...
  if(!debugMode) 
    logicWCBarrelBorderRing->SetVisAttributes(G4VisAttributes(false)); 
  //----------------------------------------------------
...
// used for RayTracer
 if (Vis_Choice == "RayTracer"){
  ...
  if(!debugMode){
		...
		logicWCBarrelBorderCell->SetVisAttributes(G4VisAttributes(false));}
  else {
        G4VisAttributes* tmpVisAtt = new G4VisAttributes(G4Colour(1.,0.5,0.5));
        tmpVisAtt->SetForceWireframe(true);
        logicWCBarrelBorderCell->SetVisAttributes(tmpVisAtt);
...
// used for OGLSX
  ...
  if(!(WCBarrelRingNPhi*WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)){
    ...
    logicWCExtraBorderCell->SetVisAttributes(G4VisAttributes(false));
...
// used for RayTracer
 if (Vis_Choice == "RayTracer"){
  if(!debugMode){  
    ...
    logicWCCap->SetVisAttributes(G4VisAttributes(false));
...
// used for OGLSX
 if (Vis_Choice == "OGLSX"){
   ...
   = new G4VisAttributes(G4Colour(0.9,0.2,0.2));
	...
	if(!debugMode)
        logicWCCapBlackSheet->SetVisAttributes(G4VisAttributes(false));
    else
...
// used for RayTracer (makes the caps blacksheet yellow)
 if (Vis_Choice == "RayTracer"){
   ...
   = new G4VisAttributes(G4Colour(1.0,1.0,0.0));
	...
	if(!debugMode)
       //logicWCCapBlackSheet->SetVisAttributes(G4VisAttributes(false)); //Use this line if you want to make the blacksheet on the caps invisible to view through
	   logicWCCapBlackSheet->SetVisAttributes(WCCapBlackSheetVisAtt);
    else
        logicWCCapBlackSheet->SetVisAttributes(WCCapBlackSheetVisAtt);}
